...
import { SKETCH_SNAP_DISTANCE, toWorldPoint } from "@/utils";
import {
  clampAngleRadius,
...
export function buildDimensionRelationPreview({
  ...
  if (firstLine && (!firstLine.is_construction || allowConstruction)) {
    for (const line of sketchParameters.lines) {
      ...
      let candidate: DimensionRelationPreviewResult | null = null;
    ...
    if (filter.snap_nearest) {
      for (const circle of sketchParameters.circles) {
        ...
        const candidate: DimensionRelationPreviewResult = {
          relation: {
            kind: "circle_line_distance",
            firstEntityId,
