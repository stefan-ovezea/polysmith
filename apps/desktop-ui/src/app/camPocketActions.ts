...
// Contextual 2D-pocket trigger: requires a selected body face (the
// pocket floor).  Same witness capture flow as face milling — islands
// are added later through the operation panel's pick flow.
export async function triggerCamPocket({
  document,
  setupId,
  runAction,
  camOperationCreate,
  ...
  if (!document) {
    addMessage(translate("cam.pocket.noDocument"));
    pushToast("warn", translate("cam.pocket.noDocument"));
    return;
  }
  const faceId = document.selected_face_id;
  if (!faceId) {
    addMessage(translate("cam.pocket.noSelection"));
    pushToast("warn", translate("cam.pocket.noSelection"));
    return;
  }
  ...
  if (!reference) {
    addMessage(translate("cam.pocket.captureFailed"));
    pushToast("error", translate("cam.pocket.captureFailed"));
    return;
  }
  ...
  const operation: CamOperationPayload = {
    ...
    // Same defaults as face milling; no default stepdown — a cleared
    // stepdown means single-pass (the face level only).
    parameters: {
      spindle_rpm: 8000,
