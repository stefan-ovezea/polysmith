...
// formed by two entities — two `SketchLine`s, a line and a
// `SketchArc`, or two arcs — with a tangent `SketchArc`, trimming
// each operand back to its tangent point. The fillet is parametric:
// operands A and B store their *trim* endpoint (not the original
// corner) and the arc's two endpoints, but the `corner_vertex_id`
// is preserved so the relationship can be re-solved on every
// recompute (when one of the operands moves) and fully reverted on
...
struct SketchFillet {
  ...
  double corner_x;
  double corner_y;
  // Operands, side A and side B. Exactly one of {line_a_id, arc_a_id}
  // is non-empty (same for side B) — the kind is derived: a non-empty
  // arc id makes that operand an arc. Line-line records (v1) leave
  // both arc ids empty; serialization omits them so old builds keep
  // loading line-line files.
