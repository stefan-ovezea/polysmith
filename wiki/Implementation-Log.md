...
- entity mapping: LINE → line, CIRCLE → circle, ARC → arc (libdxfrw stores angles in radians; equal start/end angles = full circle), LWPOLYLINE/POLYLINE → per-vertex segments with **bulge→arc conversion** (r = L·(1+b²)/(4|b|), center at signed height L·(1−b²)/(4b) from the chord midpoint along the left normal; b>0 = CCW), closed-flag closing segment; POINT → `projected_points` (survives the vertex rebuild); SPLINE → 64 straight segments via de Boor; ELLIPSE → circle when ratio≈1, else 64 segments (libdxfrw's `secPoint` is the major-axis vector relative to the center). TEXT/MTEXT/HATCH/INSERT/dimension/etc. and non-planar/degenerate entities are skipped and counted, reported via a structured-log warning and the feature's `parameters_summary` ("N entities · skipped M"). Entities inside block definitions are ignored; INSERTs are skipped (v1 — no block resolution).
- **imported geometry stays exact**: `add_sketch_line` gained an `infer_constraints` flag (default true — existing callers unchanged). DXF import passes false plus `snap_start=false`: without it, near-axis segments got an inferred H/V constraint that SNAPPED the coordinates axis-aligned, deforming tessellated splines/ellipses (found by the spline regression test). Shared exact endpoints still coalesce via `find_coincident_endpoint`, so polylines chain into profiles.
- **imported geometry stays editable**: unlike projection (which locks derived points), DXF import does NOT fix any vertices — fixed flags would clutter the canvas with fix badges on every endpoint and break later constraint edits ("Cannot make a line horizontal when both endpoints are fixed"). Imported POINTs ride the `projected_points` storage (the only standalone-point storage that survives the vertex rebuild) but are explicitly unfixed after the first refresh; `sync_fixed_point_flags` preserves the unfix across later refreshes. Pinned by the "no fixed vertices" assertions in the test suite.
- units: `$INSUNITS == 1` (inches) scales imported geometry ×25.4 into the mm document (logged); anything else imports raw. Export writes `$INSUNITS` from document units (mm→4, in→1) plus `$EXTMIN`/`$EXTMAX`.
- added `export_document_dxf { file_path }` — writes the ACTIVE sketch as an ASCII R2013 (AC1027) DXF (`src/dxf/dxf_export.cpp`, `DxfWriteInterface`): lines → LINE, circles → CIRCLE, arcs → ARC (start/end angles recomputed from cached endpoints via atan2, radians — the writer converts to degrees; CW sketch arcs are swapped to the DXF CCW convention), projected points → POINT. Polygon records are not re-emitted (they already exist as constituent lines). Throws "No active sketch" when nothing is active; `document_exported` carries `format: "dxf"` (TS event schema enum extended).
- build-system fix: the dxfrw static-lib target was missing `intern/dwgBuffer.cpp` and `intern/dwgutil.cpp` — plain byte-buffer/hex helpers the entity classes reference from their (unused in DXF builds) parseDwg paths. Previous builds only linked because a stale lib from an earlier source set was on disk; consuming any DRW_Entity subclass from the app exposed the missing symbols. Neither file has iconv/DWG-reader dependencies, so the DXF-only subset still holds.
- sanity gate: libdxfrw's ASCII `readCode()` runs `atoi()` on the first line, so arbitrary text "parses" as group-code 0 and read() reports success with zero entities — `import_dxf_geometry` now rejects files whose first non-empty line isn't numeric (or lacking the binary-DXF sentinel) up front.
- UI: File menu + header ribbon gain Import DXF (open dialog) and Export DXF (save dialog) items (`documentDialogs.ts`, `AppHeader.tsx`, `AppTopBar.tsx`, i18n `header.importDxf`/`header.exportDxf`/`dialogs.*`); `useCadCore` gains `importDxf`/`exportDocumentDxf`.
- tests: new `cad_core_dxf_import_export_test` suite (15 cases): basic entities, inches scaling, bulge arcs (both signs), closed LWPOLYLINE + legacy POLYLINE, spline tessellation endpoints, near-axis-line exactness, circular/elliptical ellipses, skipped-entity counting, projected-point persistence through refreshes, export/import round-trip (1e-6), export without sketch, missing/garbage files, save/load round-trip, custom plane id + invalid plane. Hand-assembled ASCII DXF fixtures (R12 group codes) in the temp directory — no external test files.
- v1 limitations (deliberate): TEXT/MTEXT/DIMENSION/LEADER annotations skipped (iconv-free textcodec stub passes system-codepage bytes); INSERT blocks not resolved (their internals are ignored); polygons export as constituent LINEs; SPLINE/ELLIPSE approximated by 64 segments; HATCH skipped.
...
### STL import + mesh body features

- added `import_stl { file_path, scale? }` — a new `mesh_import` feature kind storing ONLY the source path (+ scale); the core re-reads the STL from disk on every compile (`build_mesh_import_shape`: `StlAPI_Reader` + `BRepMesh_IncrementalMesh` so every face carries a triangulation), and a missing file degrades the feature with `dependency_broken` + a timeline warning instead of crashing
- added `convert_mesh_to_body { body_id }` — a new `mesh_to_body` feature that converts the mesh into a regular solid body ALONGSIDE it (sew → `BRepBuilderAPI_MakeSolid` → `ShapeFix_Solid` → `ShapeUpgrade_UnifySameDomain` with a 50k-face guard); non-watertight meshes return a structured error. The converted solid is snapshotted at creation (the standalone body-copy pattern), so it is independent afterwards — the source mesh body can be deleted without losing the solid
- added `project_body_into_sketch { body_id, mode }` — projects a mesh body onto the active sketch plane as fixed-endpoint sketch lines with a live `SketchProjection` record (`source_kind = "body"`, source id encodes the mode so both modes coexist); works on origin ref-plane sketches via the legacy frame fallback
  - `section`: `BRepAlgoAPI_Section` between the mesh and the sketch plane, with a coplanar-face boundary fallback for bodies sitting flat in the plane
  - `silhouette`: mesh-native outline (adjacent-triangle normal sign test + boundary edges + collinear merging + duplicate-segment/polyline dedupe) projected along the plane normal — the exact `HLRBRep_Algo` pipeline crashed with memory corruption on mesh input, so the manual criterion is used instead
- body compiler: `mesh_import`/`mesh_to_body` force the native mesh path and insert via the modifier replay (never the compound-fuse path); fillet/chamfer/shell/hole target resolution now skips non-solid bodies so mesh shells can never reach solid-only operators
- serialization: both new parameter structs round-trip through `.polysmith`; dependency walker checks file existence (`mesh_import`) and source-body availability (`mesh_to_body`); `refresh_sketch_projections` gained the `body` branch and now materializes the origin frame for ref-plane sketches
- UI: Import Mesh button in the header ribbon + File menu (STL file dialog), mesh bodies appear in the Bodies browser (eye/rename/context menu) with a "Convert to Body" action, and the Project tool gains a Section | Silhouette segmented control — with the tool armed, clicking a mesh body projects the whole body
- added `cad_core_stl_import_test` (new suite, 8 cases): import/compile/viewport, section on ref-plane, coplanar-section fallback, silhouette outline, mode coexistence, conversion (solid + face unification), missing-file degradation + recovery, and save/load round-trip
- v1 limitations (deliberate): curved STL edges project as polylines (no arc recovery); mesh bodies are shells — no booleans/fillets/chamfers/holes on them; STL assumed mm (scale available on the command/AI path); no file drag-drop, no mesh editing

### Mesh-document performance pass

- `to_payload` gained an `include_opaque` flag (default false): `document_state` events strip the opaque B-rep shape snapshots (`mesh_to_body` and standalone `body_copy`) the UI never reads, while save/load passes true and persists them — a converted-body document went from a ~236 KB to ~104 KB event payload (measured against a real saved part), removing the per-command JSON parse/render cost that made the UI sluggish after importing and converting a mesh
- face outlines now merge collinear runs before projection (`merge_collinear_outline` in the face-outline wire helpers): converted-mesh faces carry one boundary edge per original triangle, so projecting such a face emitted one sketch line per triangle edge (75 lines for one face in the reported case) — collinear runs collapse back to the real corners (regression case: an L-shaped plate's bottom face merges to exactly 6 corners)
- note: existing face projections created before the merge change will flag "Re-project to update" on the next recompute (the outline corner count changed) — re-projecting once picks up the merged geometry
- added Douglas-Peucker simplification to face outlines (`simplify_outline_polyline`, 0.05 mm — below STL facet resolution and print tolerance) and to the mesh section/silhouette polylines, so chord-sampled curved outlines no longer flood sketches with dense lines (a 91-point quarter-circle decimates to ~27 points with every original point guaranteed within tolerance)
- added `detach_body_projections { body_id }` — removes every live projection link sourced from a body (face/edge/vertex/body projections) while keeping the generated fixed sketch lines, so the source body (e.g. a converted mesh body) can be deleted without breaking the sketches and extrudes built on the projection; exposed as "Detach Projections" in the body context menu when the body sources a projection
- projection paths now create sketch entities WITHOUT automatic dimensions (`auto_dimensions = false` on the sketch primitives): projected lines/circles/arcs are derived geometry with fixed endpoints, and the per-line length + angle auto-dimensions that user-drawn lines get were flooding projected sketches (a 75-line projection carried 150 driving dimensions) and over-constraining the planegcs solver — the source of the reported sluggishness after projecting a converted mesh
- deleting a feature now freezes construction planes that reference it: the plane keeps its cached frame, severs the source reference (`plane_type = "detached"`), and stays healthy, so sketches hosted on it survive (reported: deleting the converted body left the offset plane dangling). The construction-plane refresh also auto-freezes planes whose source feature no longer exists anywhere in the document, which repairs already-broken saved documents on their first edit after load
- the TS zod schema and type for `construction_plane_parameters.plane_type` accept the new "detached" value

### Projection performance pass (real-world STL, 1182-face converted panel)

Measured with a real fan-panel STL: projecting a face used to hang the core for 20+ minutes. Fixes:

- **Bulk insertion**: projection commands now add sketch entities with `refresh_immediately = false` and run two derived-state refreshes for the whole batch (one to build vertex records, one after fixing endpoints with `set_sketch_vertex_fixed(..., refresh_immediately = false)`) instead of one refresh per entity — per-entity refreshes made large outlines O(n²) and froze the core. All projection paths (face/edge/profile/body) share the pattern.
- **Complexity cap**: with bulk insertion the projection is near-linear and the planegcs solver is skipped entirely for constraint-free projected sketches, so the cap is a degenerate-case guard only (5000 corners). Measured end-to-end on the user's fan-panel STL: the 910-corner fan face projects in ~2.8s (910 lines, 0 dimensions, 41 profiles) and the re-project dedup is ~1ms.
- **Triangulations in the snapshot**: `convert_mesh_to_solid` meshes the result before the creation-time snapshot is taken (`UnifySameDomain` rebuilds faces and drops triangulations), so deserialized converted bodies skip re-meshing.
- **`compile_bodies(document, include_meshes = false)`**: shape-only consumers (face/edge resolution, dependency checks, export collection, selection/extrude helpers, the refresh walker's prefix compiles) skip the viewport tessellation pass — for a converted-mesh document that pass costs hundreds of ms per compile.
- Remaining known cost on this part: parsing the ~2.8MB ASCII B-rep snapshot per compile (cold ~600ms, warm ~70ms on the dev laptop) — a binary snapshot format is the natural follow-up.

### Viewport cost pass for faceted bodies (same fan panel)

The viewport state build was the next bottleneck after projecting (measured 17s cold). Fixes:

- `derive_face_frame` no longer runs `BRepGProp_Face::Normal` numerical integration for planar faces (it was discarded by the plane branch anyway) — the dominant per-face cost (17s → 8s on the fan panel).
- `mesh_to_body`/`mesh_import` bodies skip edge/vertex pick primitives (facet edges are useless for fillet picking). Face records are kept (sketch-on-face and face projection need them) but with decimated pick geometry: faces over 48 triangles emit a strided subset with positions compacted to the used nodes. Payload dropped from 10.7MB to ~6MB (the remainder is the rendered body mesh itself, which is inherent).
- face pick geometry for large mesh faces was first decimated by striding (only ~7% raycast coverage — clicks mostly missed), then rebuilt as a centroid fan over the face's outer wire: full-face coverage, exact for planar mesh faces, holes included
- `project_body_into_sketch` now also accepts `mesh_to_body` bodies (resolving their shape through the compiled bodies) — projecting the CONVERTED body's section/silhouette works (verified against the fan-panel STL: silhouette 3.8s, section 7.1s). The projection refresh handles converted sources the same way.
- a duplicate-segment guard makes a projection a no-op when every segment already exists in the sketch (e.g. an imported mesh and its converted solid share the same footprint) — inserting coincident lines corrupts the sketch's vertex rebuild. The old "both modes coexist" test expectation (coincident section+silhouette of a box) was updated to assert the no-op instead.
- RESOLVED (was a heap corruption, 0xC0000409, in the in-suite convert/no-op sequence): under an ASAN build the sequence now runs clean, and the in-suite test is restored (tests 14–18 pin the no-op behaviour across L-shape, coplanar, silhouette-first, and convert-first variants). The crash is attributed to the coincident-line corruption path below, now fixed.
- FIXED projection corruption (the "partial duplicate" leak): `compute_mesh_section_polylines` on a mesh body sitting flat on the sketch plane leaked INTERNAL coplanar edges (triangle diagonals, plate seams) — both from the BOP section (which emits every coincident face's wire) and from the coplanar-boundary fallback. The converted solid's unified face yields the clean outline, so projecting the converted body after the mesh body produced a PARTIAL duplicate: the no-op guard missed 2 of 6 segments and inserted all of them — coincident lines, corrupted sketch. The section now filters edges shared by two coplanar faces out of both sources and always unions the region-boundary walk with the BOP section (also fixes partly-coplanar bodies like the fan panel, whose coplanar outline was missing when the BOP found blade crossings). The mesh-native silhouette's edge-on distinct-normal threshold moved from 1e-24 to 1e-8 so float32 normal wobble (~1e-14) on coplanar triangles no longer leaks triangulation edges while real corners (~3e-4+) still pass.
- Measured after the fixes: projection 2.8s, viewport cold build ~7.7s, warm build <1ms — the identical topology code runs sub-millisecond when warm, so the cold number is dominated by laptop CPU throttling after the compile/tessellation burst rather than algorithmic cost.
...
### Sketch auto-dimension regression fix (sketch/dimmensions)

The draft dimension badges regressed after the 2026-06 refactor: the
preview window could sit on top of the point where the user was placing
the end of a line and swallow the click (the 2026-05-28
`pointer-events-none` fix had been applied only to the post-commit
editor, never to the draft boxes); pointer moves re-focused +
re-selected the badge input every frame (focus flapping + OS cursor
flicker read as unexplained drag jerk); arcs showed a mislabelled
"length" badge and no dimension at all (the emitter skipped arc auto
dims unconditionally while the draft had one); ellipses had no preview
dimension whatsoever.

- **Click-through + fade** — NOTE: the `pointer-events-none` badge +
  pointer-proximity fade were tried and REVERTED during the session:
  click-through badges let a finishing click fall through to the canvas
  and commit the draft mid-edit (a three-point arc's third click).
  The badge form stays `pointer-events-auto`; placement safety comes
  from position (badges anchor away from the cursor). Focus-once guard
  in `focusDraftField` (no per-move focus/select churn) stayed.
- **Anchor restoration** — the rectangle length box moved off the
  cursor to the far edge midpoint (a693745 convention); the deleted
  rationale comments (f9da570 anti-trap, origin anchoring) restored.
- **Arc** — the draft badge is the arc's single dimension window: the
  `length` field shows the aim distance at stage 1 and the chord
  between the ends at stage 2 (`secondPoint`/`toolMode` carried on the
  session); typed values reshape the live draft and commit;
  post-commit follows the circle convention (auto dim deleted unless
  typed; typed/parameter value becomes a driving dim — the core's
  `update_sketch_dimension` arc_radius path, no new core command).
  The planned radial scene dimension (leader + arrowhead + label at
  the sweep mid-angle via a shared `computeArcPreviewGeometry`) was
  NOT landed — deferred.
- **Core emitter fix** — `sketch_curve_polygon_emit.inc` arc block now
  mirrors the circle block: explicit `arc_radius` wins, auto dim is
  the fallback, so a committed arc shows its radius dimension.
- **Ellipse (preview only)** — the ellipse draft got `radiusX` /
  `radiusY` boxes at the axis ends; typed values drive the live
  preview and the commit. No persistent ellipse dimension (the core
  has none) — UI-only by design decision.
- **Per-frame rebuild cache** — `renderDraftDimensions` skips the
  group dispose/rebuild when a key of all geometry inputs is
  unchanged (no more constant allocations while the pointer is idle).
- **Tests** — `cad_core_dimension_completion_test` gains
  `test_arc_auto_radius_dimension_emitted` (fail-before/pass-after on
  the emitter fix) and `test_arc_radius_update_drives_endpoints`.
  All C++ suites + `tsc --noEmit` clean.

### Arc draft double-window fix (sketch/dimmensions, same day)

User report during three_point arc creation (press first point, drag
toward the second end): TWO dimension windows appeared — one
non-editable showing the real distance "as info", one floating input
whose typed value was ignored. Investigation found the old/new mix:

1. **Old window** — `renderChordDraftDimension`
   (`draftPointerPreview.ts`) rendered a scene dimension (extension
   lines + arrows + sprite label) between the first point and the
   cursor for three_point stage 1. Scene geometry, not HTML — the
   uneditable "info" window. It predated the draft-dimension session
   system and was never removed when the HTML badge was extended to
   arcs.
2. **New window** — the `DraftDimensionFieldEditors` badge (the
   `length` field), anchored at the first click.
3. **Ignored value** — Enter at arc stage 1 was a silent no-op
   (`commitDraftDimensionSession` returned early without a
   `secondPoint`): type a value + Enter did nothing visible.

Fixes (UI-only, user-approved direction: one editable badge for both
stages):

- `renderChordDraftDimension` deleted (function + call) — the HTML
  badge is the arc's single dimension window.
- Stage-1 → stage-2 transition extracted as `advanceArcDraftSession`
  (`draftCommit.ts`), shared by the click commit and the new Enter
  path: the placed end re-projects onto the typed chord (direction
  preserved), the center_start_end lock clears (its stage-2 field
  means the chord), values recompute so the badge shows the chord
  that landed. **Enter at stage 1 now places the second end** instead
  of doing nothing.
- Stale mixed-state cleaned: the `draft_trace` arc diagnostic
  removed, the duplicated `circleTangentLineIdsRef` line fixed, the
  "radial dimension label" comments in
  `draftDimensionScreenPosition.ts` corrected, the unused `radius`
  screen-position key removed from `DraftDimensionScreenPositions`.
  The `main.tsx` localStorage crashlog stays until the ellipse crash
  is verified fixed.
- A radial scene dimension for arc stage 2 remains deferred.

Gates: `tsc --noEmit` clean; full `pnpm core:build` + `pnpm test:core`
re-run to close the previous session's open gate. AWAITING user
in-app verification (one badge during the arc draft, typed value +
Enter places the second end, no uneditable duplicate window).

### Arc radius badge restored (sketch/dimmensions, same day)

User: "the first dimension works but the second one (radius) does
not even show up." The double-window fix had left the arc with only
the chord/aim-distance field — the stage-2 radius readout the user
expected was missing entirely. Restored as the arc's second badge:

- `draftSessionFields("arc")` → `["length", "radius"]`. The radius
  value is live: circumradius of (start, secondPoint, current) via
  the shared `computeArcPreviewGeometry` for three_point, center
  distance start↔secondPoint for center_start_end; hidden (no screen
  position) at stage 1 and when the three points are collinear.
- Typed radius reshapes the draft: three_point moves the apex along
  the chord's perpendicular bisector until the circumradius matches
  (d = R + sqrt(R² − c²/4), apex side preserved; R < c/2 rejected);
  center_start_end rescales the session's arc-start point radially
  and re-clamps the end onto the new circle. The session's
  `secondPoint` now carries the reshape, so `adjustedArcSecondPoint`
  and `commitArcDraft` use it (the click-placed ref stays raw).
- Radius badge anchors at the arc center (circumcenter for
  three_point, first click for center_start_end, offset downward so
  it never overlaps the length badge).
- First keystroke over a live readout selects it first — typing "5"
  over "23.45" replaces instead of appending (shared with every
  other tool's badge).
- Post-commit: `applyArcExpression` applies a parameter expression
  typed into the radius field to the auto `dim-arc-<id>` dim (the
  arc previously fell through to the POLYGON branch);
  `shouldDeleteArc` keeps the auto dim when either field was typed.
