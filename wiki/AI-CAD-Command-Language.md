...
#### `remove_sketch_projections`

Heals a sketch's projection clutter on a sketch feature. With
`keep_geometry = false` ("Remove projections") every entity generated by any
projection record is deleted — the records, their lines/circles/arcs/projected
points, and the dimensions/constraints referencing them; hand-drawn geometry
survives. With `keep_geometry = true` ("Unlink projections") the entities
stay but the live links are dropped and the projected vertex styling is
cleared. Both modes clear `projected_sources` and the `dependency_broken`
alarm that a partially-deleted projection (record count no longer matches its
source outline) raises on the sketch.
