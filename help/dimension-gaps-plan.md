...
### 2. Horizontal / Vertical Point Distance ✅ IMPLEMENTED

**What:** Dimension between two points projected onto the X or Y axis.

**Current state:** Implemented as `point_distance` with `display_as: "x"` or `"y"`.
- `add_sketch_point_distance_dimension` accepts optional `axis` parameter
- Solver uses `addConstraintDifference` for signed X/Y constraint with
  point-ordering preservation (July 2026 fix: sign from current solver positions)
- TS `computePointDistanceAxis` auto-detects axis from cursor position
- Auto/linear mode drag-to-choose placement (drag up/down → horizontal "x",
  drag left/right → vertical "y", drag perpendicular → aligned/Euclidean)
- Viewport rendering: dimension line is axis-aligned with per-point projections
...
### Phase B: Horizontal / Vertical Point Distance ✅ DONE (June 2026)

All items implemented. Uses `point_distance` with `display_as: "x"` / `"y"`.
Solver constraint is `addConstraintDifference` with point-ordering sign
preservation. Cursor-based axis detection via `computePointDistanceAxis`.
...
- **Angular / linear mode filter** — The new dimension types should be
  filterable by mode. Arc length → `arc_length` mode.
